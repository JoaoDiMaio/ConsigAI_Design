...
export function MiniCard({ variant = 'eco', name, desc, value, detail, onNav }) {
  ...
  return (
    <div style={{ borderRadius: 21, border: `2px solid ${v.border}`, background: v.bg, overflow: 'hidden', marginBottom: 12 }}>
      <div style={{ padding: '16px 16px 0' }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, marginBottom: 12 }}>
          ...
          <div>
            <div style={{ fontSize: 13, fontWeight: 700, color: v.nameCl, lineHeight: 1.2, marginBottom: 3 }}>{name}</div>
            <div style={{ fontSize: 11, fontWeight: 600, color: v.descCl, lineHeight: 1.35 }}>{desc}</div>
          </div>
        </div>
      </div>

      <div style={{ height: 1, background: v.divider, margin: '0 16px' }} />
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 10, padding: '12px 16px' }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 2, whiteSpace: 'nowrap', color: v.valueCl }}>
            ...
            <span style={{ fontSize: 22, fontWeight: 800, lineHeight: 1, letterSpacing: '-.02em' }}>{value}</span>
          </div>
          {detail && (
            <div style={{ fontSize: 10, fontWeight: 600, color: v.detailCl, marginTop: 2 }}>{detail}</div>
          )}
        </div>
        ...
        <button
          ...
          style={{
            border: 0, borderRadius: 13, color: '#fff', background: v.btnBg,
            padding: '11px 16px', fontWeight: 700, fontSize: 12, lineHeight: 1.2,
