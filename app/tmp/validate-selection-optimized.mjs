...
for (const vp of [{name:'desktop',w:1366,h:768},{name:'mobile',w:390,h:844}]) {
  ...
  const state = await page.evaluate(() => {
    ...
    const win = document.querySelector('iframe[title="Ofertas ConsigAI"]')?.contentWindow;
  ...
  console.log(vp.name, 'interactionMs', (t1 - t0), JSON.stringify(state), 'route', page.url());
  await browser.close();
}
